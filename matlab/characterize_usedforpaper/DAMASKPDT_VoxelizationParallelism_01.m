...
%% load user data
    parm.fns = dir(['*628*GrainVxlGrids.csv']);
...
for rk=1:1:parm.N
    ...
    parm.Incr = str2double(extractBetween(parm.fn,'345.Incr.','.GrainVxlGrids.csv'));
...
%% load content
    ...
    delimiter = ';';
    startRow = 5;
    formatSpec = '%f%*s%*s%*s%*s%*s%*s%*s%*s%*s%f%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%[^\n\r]';
...
%% load raw heavy data from damaskpdt output
   ...
   NGR(1+parm.RK,3) = sumNXYZ;
      
   clearvars -except rk parm NGR;
end

fid = fopen(['DAMASKPDT.SimID.' num2str(parm.ID) '.NumberOfVxl.csv'], 'wt');
fprintf(fid, '%s;%s;%s\n','Rank;Increment;NumberOfVoxel');
