%% DAMASKPDT correlate stress tensor spread per mat point
...
%% correlate stress tensor values with distance
	...
	%     %view(46,20);
        ...
        nquants = [0.0; 0.01; 0.05; 0.25; 0.5; 0.75; 0.95; 1.0];
        nq = length(nquants);
        dbins = [0.0:0.1:24.0]./double(parm.NX); %end values
        Nbins = length(dbins)-1;
        
        %% build reference for KS Test
        dright = 0.0/double(parm.NX);
        dleft =  1.0/double(parm.NX);
        refcand = [];
        refvalues = [];
        refcand = Distances(1,:) > dright & Distances(1,:) <= dleft; 
        refvalues = StressSpatDistr(c,refcand);
   
        QUANTMATRIX = nan(Nbins,1+1+nq+1+1+1+1); %binid, cnts, quant values, accept yes/no 0.05, p@0.05, accept yes/no @0.01, p@0.01
        for d = 1:1:Nbins
            clearvars cand values;
            QUANTMATRIX(d,1) = dbins(d) * double(parm.NX);
            dright = dbins(d);
            dleft = dbins(d+1);
            ...
            if ~isempty(values)
                for q=1:1:nq
                    QUANTMATRIX(d,2+q) = quantile(values,nquants(q));
                end
                ...
                QUANTMATRIX(d,2+nq+1) = double(h5); %acce y/n
                QUANTMATRIX(d,2+nq+2) =  p5; %significance @0.05
                QUANTMATRIX(d,2+nq+3) = double(h1); %acce y/n
                QUANTMATRIX(d,2+nq+4) =  p1; %significance @0.05
                display(d);
            end
        end
