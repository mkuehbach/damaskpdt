%% DAMASKPDT compute modal orientations
...
%% load raw heavy data from damaskpdt output
    ...
    fid = fopen(parm.fn);
    GrainIDsQuats = fread(fid,[parm.NC, parm.NR],'double'); %implicit transpose
    fclose(fid);
    display([parm.fn ' loaded']);
    clearvars -except i cs ss parm GrainIDsQuats;
...
%% grain resolved orientation spread
    ...
    cGrainIDsModalOris = unique(GrainIDsQuats(1,:));
    cGrainIDsModalOris(2:5,:) = zeros(4,length(GrainIDsModalOris));
    GrainIDsEigenvalues = unique(GrainIDsQuats(1,:));
    GrainIDsEigenvalues(2:5,:) = zeros(4,length(GrainIDsModalOris));
    cGrainIDsEigenvalues = unique(GrainIDsQuats(1,:));
    cGrainIDsEigenvalues(2:5,:) = zeros(4,length(GrainIDsModalOris));
    
    %% extract all orientation values from matpoints of grain
    for j=1:1:length(GrainIDsModalOris(1,:))
        gid = GrainIDsModalOris(1,j);
        qin = [];
        qin = GrainIDsQuats(2:5,GrainIDsQuats(1,:)==gid);
        Quats = [];
        cQuats = [];
        %damask passive take as is
        Quats = orientation(quaternion(qin(1,:),qin(2,:),qin(3,:),qin(4,:)),cs,ss);
        % damask passive to active
        cQuats = orientation(quaternion(qin(1,:),-qin(2,:),-qin(3,:),-qin(4,:)),cs,ss); %switch from passive to active
        ...
        [cm, clambda, cV] = mean(cQuats);
        GrainIDsModalOris(2:5,j) = [m.a;m.b;m.c;m.d];
        cGrainIDsModalOris(2:5,j) = [cm.a;cm.b;cm.c;cm.d];
        GrainIDsEigenvalues(2:5,j) = V;
        cGrainIDsEigenvalues(2:5,j) = cV;
...
%         end 
        ...
        clearvars gid qin Quats cQuats m lambda V cm clambda cV;
        display(num2str(j));
    end
    display('Grain modal orientations from matpoints computed');
    
    clearvars -except i cs ss parm GrainIDsModalOris cGrainIDsModalOris GrainIDsEigenvalues cGrainIDsEigenvalues;
