...
hedgesAABB::hedgesAABB(const trpl lowerBound_, const trpl upperBound_)
{
...
voro_real hedgesAABB::computeSurfaceArea() const
{
	// Calculate the surface area of the 3D AABB.
	voro_real wx = upperBound.a - lowerBound.a;
	voro_real wy = upperBound.b - lowerBound.b;
	voro_real wz = upperBound.c - lowerBound.c;
	return 2.f * (wx*wy + wx*wz + wy*wz);
}

voro_real hedgesAABB::getSurfaceArea() const
{
    return surfaceArea;
}

void hedgesAABB::merge(const hedgesAABB& aabb1, const hedgesAABB& aabb2)
{
...
bool hedgesAABB::contains(const hedgesAABB& aabb) const
{
...
bool hedgesAABB::overlaps(const hedgesAABB& aabb) const
{
	//assert not required because aabb and myself both 3d
...
void Tree::insertParticle(unsigned int particle, const trpl position, voro_real radius)
{
	...
	if ( insertLeaf(node) == TREE_ERROR ) {
		cerr << "Tree leaf insertion error!" << "\n";
	}
...
vector<unsigned int> Tree::query(unsigned int particle, const hedgesAABB& aabb)
{
    ...
    while (stack.size() > 0)
    {
        ...
        // Copy the AABB.
        hedgesAABB nodeAABB = nodes[node].aabb;
...
vector<unsigned int> Tree::query(const hedgesAABB& aabb)
{
...
const hedgesAABB& Tree::getAABB(unsigned int particle)
{
...
unsigned int Tree::insertLeaf(unsigned int leaf)
{
    ...
    hedgesAABB leafAABB = nodes[leaf].aabb;
    ...
    while (!nodes[index].isLeaf())
    {
        ...
        voro_real surfaceArea = nodes[index].aabb.getSurfaceArea();

        hedgesAABB combinedAABB;
        combinedAABB.merge(nodes[index].aabb, leafAABB);
        voro_real combinedSurfaceArea = combinedAABB.getSurfaceArea();

        // Cost of creating a new parent for this node and the new leaf.
        voro_real cost = 2.0 * combinedSurfaceArea;

        // Minimum cost of pushing the leaf further down the tree.
        voro_real inheritanceCost = 2.0 * (combinedSurfaceArea - surfaceArea);
        ...
        else
        {
            hedgesAABB aabb;
            aabb.merge(leafAABB, nodes[left].aabb);
            voro_real oldArea = nodes[left].aabb.getSurfaceArea();
            voro_real newArea = aabb.getSurfaceArea();
        ...
        else
        {
            hedgesAABB aabb;
            aabb.merge(leafAABB, nodes[right].aabb);
            voro_real oldArea = nodes[right].aabb.getSurfaceArea();
            voro_real newArea = aabb.getSurfaceArea();
...
voro_real Tree::computeSurfaceAreaRatio() const
{
    if (root == NULL_NODE)
    	return 0.0;

    voro_real rootArea = nodes[root].aabb.computeSurfaceArea();
    voro_real totalArea = 0.0;
...
void Tree::report_tree( const string csv_fn )
{
	...
	if ( csvlog.is_open() == false ) {
		cerr << "AABBTree::Unable to open file " << csv_fn << "\n";
		return;
	}
